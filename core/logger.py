...
def safe_print(msg: str, file=None, flush=True):
    """安全的 print 函数，避免 Windows 控制台编码错误

    Args:
        msg: 要打印的消息
        file: 输出流，默认为 sys.stdout
        flush: 是否立即刷新缓冲区，默认为 True

    特点：
    1. 捕获 UnicodeEncodeError 和 OSError，避免程序崩溃
    2. 在 Windows 平台上使用 UTF-8 编码
    3. 编码失败时静默跳过，不中断程序运行
    """
    if file is None:
        file = sys.stdout
...
def setup_console_encoding():
    ...
    在程序启动时调用此函数，确保控制台能正确显示中文和 emoji。

    特点：
    1. 仅在 Windows 平台上执行
    2. 使用 chcp 65001 命令设置 UTF-8 编码
    3. 重新配置 sys.stdout/stderr 为 UTF-8 编码
    4. 失败时不影响程序运行
    """
    if sys.platform == 'win32':
        try:
            ...
            # 2. 重新配置 sys.stdout 为 UTF-8 编码（支持 emoji）
...
class Logger:
    ...
    def _get_console_level(self) -> int:
        ...
        默认 WARNING，避免 GUI 启动日志淹没关键信息。
        可通过环境变量 UE_TOOLKIT_CONSOLE_LOG_LEVEL 覆盖：
        DEBUG/INFO/WARNING/ERROR/CRITICAL
        """
        level_name = os.getenv("UE_TOOLKIT_CONSOLE_LOG_LEVEL", "WARNING").upper()

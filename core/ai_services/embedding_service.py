        ...
        with EmbeddingService._lock:
            ...
            try:
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    bundled_model_path = os.path.join(base_path, 'sentence_transformers', 'BAAI_bge-small-zh-v1.5')
                    
                    if os.path.exists(bundled_model_path):
                        print(f"[EmbeddingService] Loading bundled model from: {bundled_model_path}", flush=True)
                        from sentence_transformers import SentenceTransformer
                        EmbeddingService._embedder = SentenceTransformer(bundled_model_path)
                        print(f"[EmbeddingService] Bundled model loaded successfully", flush=True)
                        print(f"[EmbeddingService] Vector dimension: {EmbeddingService._embedder.get_sentence_embedding_dimension()}", flush=True)
                        return
                    else:
                        print(f"[EmbeddingService] Bundled model not found at: {bundled_model_path}", flush=True)
                
                # 开发环境或未打包模型：仅使用本地缓存（不自动下载）
                # ⚡ 优化：设置离线模式，只使用本地缓存
                os.environ["HF_HUB_OFFLINE"] = "1"
                os.environ["TRANSFORMERS_OFFLINE"] = "1"
                
                print(f"[EmbeddingService] Loading model: {self._model_name} (offline mode only)...", flush=True)
                from sentence_transformers import SentenceTransformer
                
                try:
                    # 仅尝试离线加载，不自动下载
                    EmbeddingService._embedder = SentenceTransformer(self._model_name)
                    print(f"[EmbeddingService] Model loaded from cache: {self._model_name}", flush=True)
                    print(f"[EmbeddingService] Vector dimension: {EmbeddingService._embedder.get_sentence_embedding_dimension()}", flush=True)
                except Exception as offline_error:
                    # 如果离线加载失败，不再自动下载，而是提示用户手动下载
                    print(f"[EmbeddingService] Model not found in cache: {offline_error}", flush=True)
                    print(f"[EmbeddingService] Please download the model manually from Settings > AI Assistant", flush=True)

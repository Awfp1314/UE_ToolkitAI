...
from .update_checker_integration import UpdateCheckerIntegration
...
class AppBootstrap:
    ...
    def __init__(self):
        ...
        self.update_checker_integration = UpdateCheckerIntegration()
    ...
    def run(self) -> int:
        ...
        try:
            ...
            # 5.3.5: 更新检查阶段（非阻塞）
            self.logger.info("开始更新检查阶段")
            # 注意：此时主窗口还未创建，先不传递main_window参数
            self.update_checker_integration.check_for_updates_async(self.app)
            self.logger.info("更新检查已启动（异步）")

            # 5.4: 实现模块加载回调注册
            # 定义 on_progress 回调（更新 Splash，不阻塞UI）
            def on_progress(percent: int, message: str):
                """进度回调（异步更新，不阻塞UI）"""
                if self.splash:
                    self.splash.update_progress(percent, message)
                # ⚡ 移除 processEvents()，避免阻塞UI
                # 进度更新通过 splash 的内部定时器异步处理
            ...
            def on_complete(module_provider):
                ...
                try:
                    ...
                    if not success:
                        ...
                        # UI显示成功后，将主窗口引用传递给更新检测器
                        if self.ui_launcher.main_window:
                            self.update_checker_integration.set_main_window(self.ui_launcher.main_window)
                            self.logger.info("已将主窗口引用传递给更新检测器")
    ...
    def _cleanup(self):
        ...
        try:
            ...
            # 等待更新检查线程完成（如果存在）
            if hasattr(self, 'update_checker_integration') and self.update_checker_integration:
                try:
                    self.update_checker_integration.cleanup()
                    self.logger.info("更新检查线程已清理")
                except Exception as e:
                    self.logger.debug(f"清理更新检查线程失败: {e}")

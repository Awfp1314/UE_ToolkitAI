...
class BaseLogic(ABC):
    ...
    def _load_config(self):
        ...
        try:
            if self.config_file.exists():
                try:
                    self.config = ConfigUtils.read_json(self.config_file, default={})
    ...
    def save_config(self) -> bool:
        ...
        try:
            ...
            # 使用 ConfigUtils 保存配置（原子写入由 ConfigUtils 处理）
            ConfigUtils.write_json(self.config_file, self.config, create_backup=False, indent=2)
    ...
    def export_config(self, output_path: Path) -> bool:
        ...
        try:
            ...
            ConfigUtils.write_json(output_path, self.config, create_backup=False, indent=2)
    ...
    def import_config(self, input_path: Path) -> bool:
        ...
        try:
            ...
            try:
                imported_config = ConfigUtils.read_json(input_path, default=None)
                if imported_config is None:
                    self.logger.error("导入的配置文件为空或无效")
                    return False
            except ConfigError as e:
                self.logger.error(f"导入的配置文件格式错误: {e}")
                return False

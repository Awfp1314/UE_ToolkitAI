...
class ConfigManager:
    ...
    def load_template(self) -> Dict[str, Any]:
        ...
        try:
            template = ConfigUtils.read_json(self.template_path, default={})
            if template is None:
                template = {}
    ...
    def load_user_config(self) -> Dict[str, Any]:
        ...
        try:
            config = ConfigUtils.read_json(self.user_config_path, default={})
            if config is None:
                config = {}
    ...
    def save_user_config(self, config: Dict[str, Any], backup_reason: str = "manual_save") -> bool:
        ...
        try:
            ...
            if self.user_config_path.exists():
                self.logger.debug(f"备份当前有效配置，原因: {backup_reason}")
                try:
                    # 读取当前配置进行备份
                    current_config = self.load_user_config()
                    if current_config:
                        if not self.backup_manager.backup_config(reason=backup_reason, config=current_config):
                            self.logger.warning("备份当前配置失败，但继续保存新配置")
                except Exception as backup_error:
                    self.logger.warning(f"备份当前配置时出错: {backup_error}，但继续保存新配置")
            
            # 使用 ConfigUtils 保存配置（不创建备份，因为我们已经手动备份了）
            ConfigUtils.write_json(self.user_config_path, config, create_backup=False, indent=2)
    ...
    def save_user_config_fast(self, config: Dict[str, Any]) -> bool:
        """快速保存用户配置（跳过备份和验证，适用于频繁保存的轻量级配置）
        ...
        try:
            ...
            # 使用 ConfigUtils 直接写入，跳过备份
            ConfigUtils.write_json(self.user_config_path, config, create_backup=False, indent=2)

            # 清除缓存（配置已更新）
            self.clear_cache()

            self.logger.debug(f"快速保存用户配置成功: {self.user_config_path}")
            return True

        except ConfigError as e:
            self.logger.error(f"快速保存用户配置失败: {e}")
            return False
        except Exception as e:
            self.logger.error(f"快速保存用户配置失败: {e}")
            return False
    ...
    def restore_from_backup(self, backup_index: int = 0) -> bool:
        ...
        if self.user_config_path.exists():
            ...
            try:
                current_config = ConfigUtils.read_json(self.user_config_path, default={})
                if current_config:

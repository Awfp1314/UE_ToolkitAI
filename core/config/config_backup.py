...
class ConfigBackupManager:
        ...
        try:
            ...
            if config is None:
                if not self.user_config_path.exists():
                    ...
                    return True
                try:
                    config = ConfigUtils.read_json(self.user_config_path, default=None)
                    if config is None:
                        self.logger.error("读取现有配置文件失败：文件为空或不存在")
                        return False
            ...
            # 使用 ConfigUtils 备份配置（不创建额外备份）
            try:
                ConfigUtils.write_json(backup_path, config, create_backup=False, indent=2)
            except ConfigError as e:
    ...
    def _verify_backup(self, backup_path: Path) -> bool:
        ...
        try:
            ...
            try:
                backup_config = ConfigUtils.read_json(backup_path, default=None)
                if backup_config is None:
                    self.logger.error(f"备份文件读取失败: {backup_path}")
                    return False
            except ConfigError as e:
    ...
    def restore_from_backup(self, backup_index: int = 0) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                backup_config = ConfigUtils.read_json(backup_path, default=None)
                if backup_config is None:
                    self.logger.error(f"读取备份文件失败: {backup_info['filename']}")
                    return None
            except ConfigError as e:
                self.logger.error(f"读取备份文件失败: {e}")
                return None

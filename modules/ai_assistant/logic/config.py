...
4. **验证示例**：
   ```
   # 你必须执行的检查逻辑：
   1. 调用 get_blueprint_info(blueprint_path="/Game/BP_Enemy")
   2. 遍历返回数据中的 graphs[].nodes[]
   3. 检查每个 node 的 title 字段
   4. 如果 title 包含 "InputAxis" 或 "InputAction" 或以 "Input" 开头，说明用户没有删除
   ...
   工具返回的数据示例：
   {
     "graphs": [{
       "name": "EventGraph",
       "nodes": [
         {"title": "Event BeginPlay", "class": "K2Node_Event"},
         {"title": "InputAxis MoveForward", "class": "K2Node_InputAxis"},  // ← 发现输入节点！
         {"title": "Add Movement Input", "class": "K2Node_CallFunction"}
       ]
     }]
   }
   ...
   这是玩家控制用的，敌人不需要。把它删掉，删完告诉我。"
   
   错误的回应：
   "我看了下，已经清理干净了。" ← 这是错误的！你必须检查 title 字段！

...
class ContextManager:
    ...
    def __init__(self, asset_manager_logic=None, config_tool_logic=None, site_recommendations_logic=None, runtime_context=None, user_id: str = "default", debug: bool = False, max_context_tokens: int = 4000):
        ...
        self.memory = EnhancedMemoryManager(
            ...
            persistent=False  # ⚡ 默认关闭持久化，重启后记忆清空
        )

        # ⚡ 性能优化：LRU 缓存（Requirement 2.1, 2.2, 2.3, 2.4）
        # 缓存资产、配置、文档、日志等查询结果，避免重复读取
        self._context_cache = ThreadSafeLRUCache(max_size=100, ttl=60.0)
        self.logger.info("已初始化上下文缓存（容量: 100, TTL: 60秒）")
    ...
    def build_context(self, query: str, include_system_prompt: bool = False) -> str:
        ...
        if is_chitchat:
            ...
            if is_asking_memory:
                ...
                if relevant_memories:
                    # ⚡ 适配新的返回格式（包含相似度分数）
                    memory_texts = [mem['content'] for mem in relevant_memories[:5]]
        ...
        if relevant_memories:
            for i, mem in enumerate(relevant_memories[:5], 1):
                ...
                source = mem['source']
                self.logger.info(f"  记忆 {i}: [相似度:{similarity:.3f}] [{source}] {content[:100]}...")
        else:
            self.logger.warning("未找到相关记忆（所有记忆相似度低于阈值）")

        if relevant_memories:
            # ⚡ 适配新的返回格式（包含相似度分数）
            memory_texts = [mem['content'] for mem in relevant_memories]
            # Token优化：每条记忆限制150字符，最多3条
            truncated_memories = [m[:150] + "..." if len(m) > 150 else m for m in memory_texts[:3]]
    ...
    def _build_asset_context(self, query: str) -> str:
        ...
        使用 LRU 缓存避免重复查询（Requirement 2.1, 2.2）
        ...
        """
        try:
            # ⚡ 性能优化：检查缓存（Requirement 2.1）
            cache_key = f"asset:{query}"
            cached_result = self._context_cache.get(cache_key)
            if cached_result is not None:
                self.logger.debug(f"命中资产上下文缓存: {query[:30]}...")
    ...
    def _build_document_context(self, query: str) -> str:
        ...
        try:
            # ⚡ 性能优化：检查缓存（Requirement 2.1）
            cache_key = f"document:{query}"
            cached_result = self._context_cache.get(cache_key)
            if cached_result is not None:
                self.logger.debug(f"命中文档上下文缓存: {query[:30]}...")
                return cached_result
    ...
    def _build_log_context(self, query: str) -> str:
        ...
        try:
            # ⚡ 性能优化：检查缓存（Requirement 2.1）
            cache_key = f"log:{query}"
            cached_result = self._context_cache.get(cache_key)
            if cached_result is not None:
                self.logger.debug(f"命中日志上下文缓存: {query[:30]}...")
                return cached_result
    ...
    def _build_config_context(self, query: str) -> str:
        ...
        try:
            # ⚡ 性能优化：检查缓存（Requirement 2.1）
            cache_key = f"config:{query}"
            cached_result = self._context_cache.get(cache_key)
            if cached_result is not None:
                self.logger.debug(f"命中配置上下文缓存: {query[:30]}...")
                return cached_result
...
你拥有记忆能力，能记住之前的对话内容，并结合用户习惯提供个性化建议。"""
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        """获取缓存统计信息（Requirement 2.4）

        Returns:
            Dict: 缓存统计信息，包括命中率、当前大小等
        """
        return self._context_cache.get_stats()

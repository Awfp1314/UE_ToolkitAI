...
class AssetReader:
    ...
    def __init__(self, asset_manager_logic=None):
        ...
        if asset_manager_logic:
            try:
                asset_count = len(asset_manager_logic.get_all_assets())
                logger.info(f"AssetReader初始化成功，资产数量: {asset_count}")
            except Exception as e:
                logger.warning(f"AssetReader初始化时无法获取资产: {e}")
        else:
            logger.warning("AssetReader初始化时asset_manager_logic为None")

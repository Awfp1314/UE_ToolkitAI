...
class ConfigToolLogic:
    ...
    def add_config_template(self, config_name: str, source_files: List[Path], description: str = "") -> bool:
        """添加新的配置模板
        ...
        try:
            ...
            # 添加配置模板到配置中，并设置路径
            desc = description if description else f"从UE工程导入的配置模板 ({copied_files}个文件)"
            template = self.add_template(config_name, desc)
            # 设置配置文件路径
    ...
    def _get_config_dir_for_project(self, project_path: Path) -> Path:
        """获取工程的配置目录路径
        
        根据工程的UE版本自动检测正确的配置目录。
        ...
        Returns:
            Path: 配置目录路径
        """
        config_base = project_path / "Saved" / "Config"
        ...
        for dir_path in possible_config_dirs:
            if dir_path.exists():
                logger.info(f"找到配置目录: {dir_path}")
                return dir_path
        
        # 如果都不存在，根据工程版本决定使用哪个
        # 检查 .uproject 文件中的引擎版本
        ...
        if uproject_files:
            try:
                ...
                with open(uproject_files[0], 'r', encoding='utf-8') as f:
                    ...
                    # UE5 使用 WindowsEditor
                    if engine_version.startswith('5'):
                        target_dir = config_base / "WindowsEditor"
                        logger.info(f"检测到 UE5 工程，使用配置目录: {target_dir}")
                        return target_dir
            except Exception as e:
                logger.warning(f"读取工程版本失败: {e}")
        
        # 默认使用 Windows（UE4 兼容）
        target_dir = config_base / "Windows"
        logger.info(f"使用默认配置目录: {target_dir}")
        return target_dir
    ...
    def copy_config_files_from_template(self, template: ConfigTemplate, target_project_path: Path) -> bool:
        ...
        try:
            ...
            # 构建目标路径 - 根据工程版本自动检测正确的配置目录
            target_dir = self._get_config_dir_for_project(target_project_path)

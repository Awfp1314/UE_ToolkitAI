...
class AssetManagerLogic(QObject):
    ...
    def __init__(self, config_dir: Path):
        ...
        # 拼音缓存字典 {asset_id: {'name_pinyin': str, 'desc_pinyin': str, 'category_pinyin': str}}
        ...
        self._backup_interval_seconds: int = 300  # 5分钟备份一次
    ...
    def _find_thumbnail_by_asset_id(self, asset_id: str) -> Optional[Path]:
        ...
        """
        if not asset_id or not self.thumbnails_dir or not self.thumbnails_dir.exists():  # type: ignore
            return None
        
        try:
            # 标准缩略图文件名格式为 {asset_id}.png
            thumbnail_path = self.thumbnails_dir / f"{asset_id}.png"  # type: ignore
    ...
    def _save_config(self) -> None:
        ...
        if current_lib_path:
            ...
            self._save_local_config(lib_config, create_backup=should_backup)

            # 更新备份记录
            if should_backup:
                self._last_backup_time = datetime.now()
                self._last_backup_asset_count = len(self.assets)

            logger.debug(f"已保存 {len(self.assets)} 个资产的配置到本地: {self.local_config_path} (备份: {should_backup})")
    ...
    def _save_local_config(self, config: Dict[str, Any], create_backup: bool = False) -> bool:
        ...
        Args:
            config: 要保存的配置字典
            create_backup: 是否创建备份（默认False，避免频繁备份导致性能问题）
...
"""
    ...
    def remove_asset(self, asset_id: str, delete_physical: bool = False) -> bool:
        ...
        try:
            ...
            if asset_id in self._pinyin_cache:
                del self._pinyin_cache[asset_id]
    ...
    def update_asset_info(self, asset_id: str, new_name: Optional[str] = None, new_category: Optional[str] = None) -> bool:
        """更新资产信息（名称和/或分类）
        ...
        if new_name is not None and new_name.strip():
            ...
            # 更新拼音缓存中的名称
            if asset.id in self._pinyin_cache:
                self._pinyin_cache[asset.id]['name_pinyin'] = self._get_pinyin(asset.name)
        ...
        if new_category is not None and new_category.strip() and new_category != old_category:
            ...
            # 更新拼音缓存中的分类
            if asset.id in self._pinyin_cache:
                self._pinyin_cache[asset.id]['category_pinyin'] = self._get_pinyin(new_category)
    ...
    def process_screenshot(self, asset_id: str, preview_project: Path) -> None:
        ...
        try:
            ...
            thumbnail_path = self.thumbnails_dir / thumbnail_filename  # type: ignore

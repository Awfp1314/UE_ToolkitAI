...
4. [`UI_REDESIGN_GUIDE.md`](UI_REDESIGN_GUIDE.md) - 开发指南
...
本项目采用模块化的 Bootstrap 启动系统，将启动流程分解为清晰的阶段：

```
main.py → AppBootstrap → 协调各阶段
...
1. **应用初始化** - 日志系统配置、QApplication 创建、单例检查
2. **UI 准备** - 主题加载、Splash Screen 显示
3. **模块加载** - 异步加载所有功能模块、建立模块依赖
4. **UI 显示** - 创建主窗口、启动单实例服务器、显示界面

**优势**：

- ✅ **清晰的职责分离** - 每个组件专注于特定任务
- ✅ **易于测试** - 独立的组件便于单元测试
- ✅ **错误处理完善** - 每个阶段都有独立的错误处理
- ✅ **便于扩展** - 新功能可以轻松插入到对应阶段
...
详细文档：[Bootstrap 系统开发者文档](docs/BOOTSTRAP_SYSTEM.md)
